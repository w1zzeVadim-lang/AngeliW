-- ==========================================
-- ЧАСТЬ 1 ИЗ 5: ОСНОВА ИНТЕРФЕЙСА И ВКЛАДКИ
-- ==========================================
...
local mf=Instance.new("Frame",sg) mf.Size,mf.Position,mf.BackgroundColor3,mf.BorderSizePixel,mf.ClipsDescendants=UDim2.new(0,430,0,260),UDim2.new(0.35,0,0.3,0),Color3.fromRGB(15,15,18),0,true
...
local function resetMainButtons()
    ...
    co1.Visible,co2.Visible,co3_1.Visible,co3_2.Visible,subNav.Visible=false,false,false,false,false
end
...
-- ==========================================
-- ЧАСТЬ 2 ИЗ 5: КНОПКИ, СЛАЙДЕРЫ И ДРАГ-СИСТЕМА
-- ==========================================
...
-- НАЗВАНИЕ И ОПИСАНИЕ ОБНОВЛЕНЫ НА ЧЕРНОЕ ПЛАМЯ
...
drag(mf) drag(open)
-- ==========================================
-- ЧАСТЬ 3 ИЗ 5: СЛАЙДЕРЫ, ПОЛЕТ И НОКЛИП
-- ==========================================
...
-- ==========================================
-- ЧАСТЬ 4 ИЗ 6: СНЕГ (СКОРОСТЬ И РАЗМЕР), ТУМАН И КОЛЬЦА
-- ==========================================
...
run.RenderStepped:Connect(function() 
    if isa and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then 
        ...
        if math.random()<0.75 then 
            ...
            -- Размер сделан аккуратным (0.6), скорость падения увеличена (от -45 до -30)
...
-- ==========================================
-- ЧАСТЬ 5 ИЗ 6: ИСПРАВЛЕННЫЙ КОРБЛОКС И ХЕДЛЕСС (ОБНОВЛЕНО)
-- ==========================================
...
local function removeKorbloxLocal()
    -- Безопасный и чистый возврат оригинальной ноги через обновление описания персонажа
    local hum = p.Character and p.Character:FindFirstChildOfClass("Humanoid")
    if hum then
        local hs = hum:GetAppliedDescription()
        if hs then hum:ApplyDescription(hs) end
    end
end
...
local function forceHeadless(head)
    ...
    if headMesh then headMesh:Destroy() end -- Полностью удаляем старый меш, чтобы скрыть голову без багов с размером
end


-- ==========================================
-- ЧАСТЬ 6 ИЗ 6: КНОПКИ ХЕДЛЕССА И РЕСПАВН (ФИНАЛ - ОБНОВЛЕНО)
-- ==========================================
...
local function clearAllVisuals()
    ...
    -- Мягкий и точный ресет внешности персонажа к его исходному состоянию
    local hum = p.Character and p.Character:FindFirstChildOfClass("Humanoid")
    if hum then
        local hs = hum:GetAppliedDescription()
        if hs then hum:ApplyDescription(hs) end
...
bHeadlessBtn.MouseButton1Click:Connect(function()
    ...
    else bHeadlessBtn.Text, bHeadlessBtn.BackgroundColor3, bHeadlessBtn.TextColor3 = "ВЫКЛ", Color3.fromRGB(60, 20, 25), Color3.fromRGB(200, 150, 150) bHeadlessBtn.UIStroke.Color = Color3.fromRGB(80, 30, 35) clearAllVisuals() if _G.korbloxActive then applyKorbloxLocal() end end
end)
...
bOnlyHeadlessBtn.MouseButton1Click:Connect(function()
    ...
    else bOnlyHeadlessBtn.Text, bOnlyHeadlessBtn.BackgroundColor3, bOnlyHeadlessBtn.TextColor3 = "ВЫКЛ", Color3.fromRGB(60, 20, 25), Color3.fromRGB(200, 150, 150) bOnlyHeadlessBtn.UIStroke.Color = Color3.fromRGB(80, 30, 35) clearAllVisuals() if _G.korbloxActive then applyKorbloxLocal() end end
end)
